...
function renderUsersTable(users) {
    ...
    tbody.innerHTML = users.map(user => {
        ...
        <tr>
            ...
            <td>
                <button class="btn-action btn-info" onclick="showResetPasswordModal('${user.user_id}', '${user.email}', '${user.person || ''}')" title="Reset Password">
...
function toggleResetPasswordVisibility() {
    const passwordInput = document.getElementById('reset-pwd-new-password');
    const eyeIcon = document.getElementById('reset-pwd-eye-icon');
...
function showResetPasswordModal(userId, email, person) {
    // Store user data for later use
    window.currentResetUser = { userId, email, person };
    
    // Populate modal with user info
    document.getElementById('reset-pwd-user-email').textContent = email;
    document.getElementById('reset-pwd-user-person').textContent = person || '-';
    
    // Reset form
    document.getElementById('reset-pwd-new-password').value = '';
    document.getElementById('reset-pwd-send-email').checked = true;
...
async function resetUserPassword() {
    ...
    const { userId } = window.currentResetUser;
    const newPassword = document.getElementById('reset-pwd-new-password').value.trim() || null;
    const sendEmail = document.getElementById('reset-pwd-send-email').checked;
    const reason = document.getElementById('reset-pwd-reason').value.trim() || null;
    ...
    try {
        const result = await api.resetPassword(userId, newPassword, reason, sendEmail);
        
        let message = 'Password reset successfully!';
        if (sendEmail) {
            message += ' User will receive an email with the new password.';
        } else if (result.temporary_password) {
            message += `\n\nTemporary password: ${result.temporary_password}\n\n⚠️ Please save this password and share it securely with the user.`;
